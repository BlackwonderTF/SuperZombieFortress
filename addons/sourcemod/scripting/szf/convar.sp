...
void ConVar_Init()
{
	...
	g_cvTankHealthMin = CreateConVar("sm_szf_tank_health_min", "3000", "Minimum amount of health the Tank can spawn with.", _, true, 0.0);
	g_cvTankHealthMax = CreateConVar("sm_szf_tank_health_max", "20000", "Maximum amount of health the Tank can spawn with.", _, true, 0.0);
	g_cvTankTime = CreateConVar("sm_szf_tank_time", "20.0", "Adjusts the damage the Tank takes per second. 0 to disable.", _, true, 0.0);
	g_cvTankStab = CreateConVar("sm_szf_tank_stab", "500", "Flat Damage dealt to the Tank from a backstab.", _, true, 0.0);
	g_cvTankDebrisLifetime = CreateConVar("sm_szf_tank_debris_lifetime", "0.0", "Amount of time (in seconds) it takes for debris thrown by Tanks to despawn. Use 0 to prevent despawning.", _, true, 0.0);
	g_cvJockeyMovementVictim = CreateConVar("sm_szf_jockey_movement_victim", "0.0", "Percentage of movement speed applied to victim from jockey grab.", _, true, 0.0);
	g_cvJockeyMovementAttacker = CreateConVar("sm_szf_jockey_movement_attacker", "1.25", "Percentage of movement speed applied to jockey during grab.", _, true, 0.0);
